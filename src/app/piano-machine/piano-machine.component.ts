...
export class PianoMachineComponent implements OnInit, AfterViewInit {
  ...
  baseSamples: Sample[] = Array.from({ length: 10 }, (_, i) => ({
    name: `Piano ${i+1}`,
    key:  `piano-${i+1}`,
    path: `assets/piano_v2/piano-${i+1}.mp3`
  }));
  ...
  private onDocMouseMove(ev: MouseEvent) {
    ...
    const rect = this.canvasRef.nativeElement.getBoundingClientRect();
    this.drawAndToggle(ev.clientX, ev.clientY, rect, '#007bff');
  }
  ...
  private onDocTouchMove(ev: TouchEvent) {
    ...
    const t    = ev.touches[0];
    const rect = this.canvasRef.nativeElement.getBoundingClientRect();
    this.drawAndToggle(t.clientX, t.clientY, rect, '#007bff');
  }
  ...
  loadSounds() {
    this.samples.forEach(s => { this.pianoSounds[s.key] = new Audio(s.path); });
  }
  initializeGrid() {
  ...
  playSound(key: string) {
    const snd = this.pianoSounds[key]; if (snd) { snd.currentTime = 0; snd.play(); }
  }
  playCurrentStep() {
